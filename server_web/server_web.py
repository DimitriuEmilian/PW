...
while True:
	...
	try:
		...
		clientsocket.sendall(b'Content-Length: ' + bytes(str(os.stat(numeFisier).st_size).encode()) + b'\r\n');
		clientsocket.sendall(b'Content-Type: ' + bytes(tipMedia.encode()) +b'\r\n');
		...
		clientsocket.sendall(b'Content-Length: ' + bytes(str(len(msg.encode('utf-8'))).encode()) + b'\r\n');
		clientsocket.sendall(b'Content-Type: text/plain; charset=utf-8\r\n');
		clientsocket.sendall(b'Server: My PW Server\r\n');
		clientsocket.sendall(b'\r\n');
		clientsocket.sendall(bytes(msg.encode()));
